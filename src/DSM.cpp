...
thread_local char *DSM::coro_buf[define::kMaxCoro];
...
DSM::DSM(const DSMConfig &conf)
  ...
  lockAddr = (uint64_t)hugePageAlloc(define::kLockChipMemSize);
  ...
  for (uint64_t i = baseAddr; i < lockAddr + define::kLockChipMemSize;
       i += 1) {
    *(char *)i = 0;
  }
...
void DSM::registerThread() {
  ...
  for(int i = 0; i < define::kMaxCoro; ++i) {
    coro_buf[i] = rdma_buffer + i * define::kPerCoroRdmaBuf;
  }
}
...
void DSM::initRDMAConnection() {
  ...
  for (int i = 0; i < NR_DIRECTORY; ++i) {
    dirCon[i] =
        new DirectoryConnection(i, (void *)baseAddr, (void*)lockAddr,conf.dsmSize * define::GB,
                                conf.machineNR, remoteInfo);
  }
...
void DSM::read(char *buffer, GlobalAddress gaddr, size_t size, bool signal,
               CoroContext * ctx) {
  /*auto test1=iCon->data[0][gaddr.nodeID];
  auto test2=(uint64_t)buffer;
  auto test3=remoteInfo[gaddr.nodeID].dsmBase;
  auto test4=gaddr.offset;
  auto test5=size;
  auto test6=iCon->cacheLKey;
  auto test7=remoteInfo[gaddr.nodeID].dsmRKey[0];
  auto test8=signal;*/
  if (ctx==nullptr) {
...
bool DSM::write_cas_sync(RdmaOpRegion &write_ror, RdmaOpRegion &cas_ror,
  ...
  return *((uint64_t*)cas_ror.source) == equal;
}

bool DSM::write_cas_parallel_sync(RdmaOpRegion &write_ror, RdmaOpRegion &cas_ror,
                    uint64_t equal, uint64_t val, CoroContext *ctx) {
  bool signal = true;
  int node_id;
  {
  ...
  if (ctx == nullptr) {
    rdmaWriteCasParallel(iCon->data[0][node_id], write_ror, cas_ror, equal, val,
                 signal);
  } else {
    rdmaWriteCasParallel(iCon->data[0][node_id], write_ror, cas_ror, equal, val, true,
                 ctx->coro_id);
    (*ctx->yield)(*ctx->master);
  }
...
bool DSM::cas_sync(GlobalAddress gaddr, uint64_t equal, uint64_t val,
                   volatile uint64_t *rdma_buffer, CoroContext *ctx) {
  cas(gaddr, equal, val, (uint64_t*)rdma_buffer, true, ctx);
...
void DSM::set_buffer_num(int buffer_num)
{
  dirAgent[0]->set_buffer_num(buffer_num);
}

void DSM::set_write_buffer_info(std::vector<uint64_t> write_buffer_ptr,int buffer_size_)
{
  dirAgent[0]->set_write_buffer_info(write_buffer_ptr,buffer_size_);
}