...
bool rdmaWriteCasParallel(ibv_qp *qp, const RdmaOpRegion &write_ror,
                  const RdmaOpRegion &cas_ror, uint64_t compare, uint64_t swap,
                  bool isSignaled, uint64_t wrID) {

  struct ibv_sge sg_cas;
  struct ibv_sge sg_write;
  struct ibv_send_wr wr_cas;
  struct ibv_send_wr wr_write;
  struct ibv_send_wr *wrBad_cas;
  struct ibv_send_wr *wrBad_write;

  fillSgeWr(sg_write, wr_write, write_ror.source, write_ror.size, write_ror.lkey);
  wr_write.opcode = IBV_WR_RDMA_WRITE;
  wr_write.wr.rdma.remote_addr = write_ror.dest;
  wr_write.wr.rdma.rkey = write_ror.remoteRKey;
  
  if (ibv_post_send(qp, &wr_write, &wrBad_write)) {
    Debug::notifyError("Send with ATOMIC_CMP_AND_SWP failed.");
    sleep(5);
    return false;
  }

  fillSgeWr(sg_cas, wr_cas, cas_ror.source, 8, cas_ror.lkey);
  wr_cas.opcode = IBV_WR_ATOMIC_CMP_AND_SWP;
  wr_cas.wr.atomic.remote_addr = cas_ror.dest;
  wr_cas.wr.atomic.rkey = cas_ror.remoteRKey;
  wr_cas.wr.atomic.compare_add = compare;
  wr_cas.wr.atomic.swap = swap;
  wr_cas.wr_id = wrID;
  if(isSignaled)
  {
    //Debug::notifyError("signaled");
    wr_cas.send_flags |= IBV_SEND_SIGNALED;
  }

  if (ibv_post_send(qp, &wr_cas, &wrBad_cas)) {
    Debug::notifyError("Send with ATOMIC_CMP_AND_SWP failed.");
    sleep(5);
    return false;
  }
