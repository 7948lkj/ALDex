...
public:
  ...
  bool write_cas_sync(RdmaOpRegion &write_ror, RdmaOpRegion &cas_ror,
                      uint64_t equal, uint64_t val, CoroContext *ctx = nullptr);

  bool write_cas_parallel_sync(RdmaOpRegion &write_ror, RdmaOpRegion &cas_ror,
                      uint64_t equal, uint64_t val, CoroContext *ctx = nullptr);
  ...
  bool cas_sync(GlobalAddress gaddr, uint64_t equal, uint64_t val,
                volatile uint64_t *rdma_buffer, CoroContext *ctx = nullptr);
...
private:
  ...
  static thread_local char *coro_buf[define::kMaxCoro];
...
public:
  ...
  char *get_cache() {return (char*)cache.data;}
  RdmaBuffer &get_rbuf(int coro_id) { return rbuf[coro_id]; }

  char *get_coro_buf(int coro_id) { return coro_buf[coro_id]; }
  

  GlobalAddress alloc(size_t size);
  GlobalAddress alloc_size(size_t size);
  GlobalAddress alloc_local_size(size_t size);
  void retrain_chain(GlobalAddress chain_start_gaddr,size_t buffer_index);
  void retrain_buffer(int node_id,size_t buffer_index);
  void set_buffer_num(int buffer_num_);
  void set_write_buffer_info(std::vector<uint64_t> write_buffer_info_,int buffer_size);

  uint64_t get_base_addr(){return baseAddr;}
...
inline GlobalAddress DSM::alloc_size(size_t size) {
  ...
  RawMessage m;
  m.type = RpcType::MALLOC_SIZE;
  m.alloc_size=size;
  

  this->rpc_call_dir(m, next_target_node, next_target_dir_id);
  auto addr = rpc_wait()->addr;


  return addr;
}

inline void DSM::retrain_chain(GlobalAddress chain_start_gaddr,size_t buffer_index) {

  thread_local int next_target_node = chain_start_gaddr.nodeID;
  thread_local int next_target_dir_id =
      (getMyThreadID() + getMyNodeID()) % NR_DIRECTORY;
  ...
  RawMessage m;
  m.type = RpcType::RETRAIN_CHAIN;
  //std::cout<<"send index: "<<buffer_index<<"send addr: "<<chain_start_gaddr.offset<<std::endl;
  m.addr=chain_start_gaddr;
  m.alloc_size=buffer_index;
...
inline void DSM::retrain_buffer(int node_id,size_t buffer_index) {

  thread_local int next_target_node =node_id;
  thread_local int next_target_dir_id =
      (getMyThreadID() + getMyNodeID()) % NR_DIRECTORY;
...
inline GlobalAddress DSM::alloc_local_size(size_t size) {

  thread_local int next_target_node =getMyNodeID();
  thread_local int next_target_dir_id =(getMyThreadID() + getMyNodeID()) % NR_DIRECTORY;
